...
public sealed record CalendarDto(
    Guid Id, string Name, string? Color, bool SupportsEvents, bool SupportsTasks, bool Shared, string? MyColor = null);

public sealed record AddressBookDto(Guid Id, string Name, string? Color, bool Shared, string? MyColor = null);
...
public sealed record TenantEmailSettingsDto(
    ...
    string? ImapUsername = null, bool HasImapPassword = false, string? ImapFolder = "INBOX");
...
public sealed record BulkResultDto(int Succeeded, int Failed, IReadOnlyList<string> Failures);
